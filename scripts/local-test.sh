...
if [[ -z "$TEST_SUITE" || "$TEST_SUITE" == "ovl-fstests" ]] && [[ ! -d "$MKOSI_FSTESTS/unionmount-testsuite" ]]; then
    echo "==> Cloning unionmount-testsuite into $MKOSI_FSTESTS/"
    git clone --depth=1 https://github.com/amir73il/unionmount-testsuite.git "$MKOSI_FSTESTS/unionmount-testsuite"
fi

echo "==> Kernel source:          $KERNEL_SRC"
echo "==> mkosi-kernel:           $MKOSI_KERNEL"
echo "==> fstests:                $MKOSI_FSTESTS"
echo "==> unionmount-testsuite:   $MKOSI_FSTESTS/unionmount-testsuite"
...
# Strip BuildSources from the drop-in — it references $GITHUB_WORKSPACE which
# doesn't exist locally.  We set it in mkosi.local.conf instead.
sed '/^BuildSources=/d' "$VFS_TEST/ci/mkosi/mkosi.conf" \
    > "$MKOSI_KERNEL/mkosi.conf.d/99-vfs.conf"

# Copy fstests profile config for non-selftests runs (mirrors CI).
# Strip BuildSources since we set the local path in mkosi.local.conf.
if [[ -z "$TEST_SUITE" || "$TEST_SUITE" != "selftests" ]]; then
    sed '/^BuildSources=/d' "$VFS_TEST/ci/mkosi/mkosi.fstests.conf" \
